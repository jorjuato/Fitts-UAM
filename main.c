...
int ExperimentFSA(void){
	...
	static Uint8 bRepeat = FALSE, cnt = 0;
	static Uint8 preOsc = 0, slopeOsc=0, postOsc=0;

	//Actualizamos variables temporales
	long int now = getmicroseconds();
	long int time_inc = now - g_init_time;

	//Actualizamos los variables de la tableta
	float x     = WAC_ABS_SCALE_X(gAbsState[WACOMFIELD_POSITION_X].nValue);
	float y     = WAC_ABS_SCALE_Y(gAbsState[WACOMFIELD_POSITION_Y].nValue);
	Uint16 press = gAbsState[WACOMFIELD_PRESSURE].nValue;
	...
	switch(g_trial_state) {
		...
		case TRIAL_STATE_INTER_TRIAL:///////////////////////////////////
			...
			else if (now - g_init_time >= INTER_TRIAL_TIME*1000)	{
		...
		case TRIAL_STATE_PRECONTACT:////////////////////////////////////
			if ( (press > MIN_PRESSURE) && IsInTarget(x,y) ) {
		...
		case TRIAL_STATE_OSCILLATION_PRESLOPE://////////////////////////
			if (press > MIN_PRESSURE) {
				if (change){
					if ( (preOsc > OSCILLATIONS_TRAIN) && !IsInTarget(x,y) )
						g_errors++;
					g_side = (g_side == 'R') ? 'L' : 'R';
					preOsc++;
				}
				if (preOsc > OSCILLATIONS_BEFORE + OSCILLATIONS_TRAIN) {
		...
		case TRIAL_STATE_OSCILLATION_POSTSLOPE://///////////////////////
			if (press > MIN_PRESSURE){
				...
				if (postOsc >=  OSCILLATIONS_AFTER) {
					if (g_errors < ((float)(preOsc + slopeOsc + postOsc - OSCILLATIONS_TRAIN) * ERROR_PERCENT) ) {
					//if (TRUE){
						ExperimentSaveData();
						g_trial_state = TRIAL_STATE_INTER_TRIAL;
						ExperimentGraphics();
	...
	if (g_trial_state > TRIAL_STATE_PREOSCILLATION){
		...
		g_TrialData[g_data_count].time = now - g_init_time;
